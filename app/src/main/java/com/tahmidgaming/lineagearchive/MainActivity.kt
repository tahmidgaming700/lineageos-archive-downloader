...
private fun ArchiveApp() {
    ...
    Scaffold(
        ...
        Box(Modifier.fillMaxSize().background(Brush.verticalGradient(listOf(MaterialTheme.colorScheme.background, MaterialTheme.colorScheme.surfaceContainer)))) {
            Column(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp)) {
                when (screen) {
                    ...
                    Screen.BUILDS -> BuildContent(selected, builds, loading, error) { file, version -> DownloadHelper.enqueue(context, file, selected?.model ?: "device", version); screen = Screen.DOWNLOADS }
                    Screen.ARCHIVE -> ArchiveContent(selected, archiveBuilds, loading, error) { summary ->
                        scope.launch {
                            loading = true; error = null
                            runCatching { LineageRepository.archiveBuild(summary.id) }.onSuccess { detail ->
                                if (detail.url != null) {
...
private fun DeviceContent(devices: List<LineageDevice>, query: String, onQuery: (String) -> Unit, loading: Boolean, error: String?, onSelect: (LineageDevice) -> Unit) {
    ...
    LazyColumn(verticalArrangement = Arrangement.spacedBy(9.dp), contentPadding = PaddingValues(vertical = 10.dp)) {
        items(devices.filter { query.isBlank() || it.name.contains(query, true) || it.model.contains(query, true) || it.oem.contains(query, true) }) { device ->
            Card(Modifier.fillMaxWidth(), onClick = { onSelect(device) }, colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface.copy(alpha = .72f))) { Column(Modifier.padding(15.dp)) { Text(device.name, fontWeight = FontWeight.SemiBold); Text("${device.oem} • ${device.model}") } }
        }
    }
}
...
private fun ArchiveContent(device: LineageDevice?, builds: List<ArchiveBuildSummary>, loading: Boolean, error: String?, onDownload: (ArchiveBuildSummary) -> Unit) {
    ...
    error?.let { Text(it, color = MaterialTheme.colorScheme.error) }; if (loading) LinearProgressIndicator(Modifier.fillMaxWidth())
    LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(vertical = 10.dp)) {
        items(builds) { build -> Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface.copy(alpha = .76f))) { Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
            Text(build.filename, fontWeight = FontWeight.SemiBold); Text("${archiveVersion(build.filename) ?: "LineageOS"} • ${archiveDate(build.filename)?.let(::formatDate) ?: "date unknown"}"); Button(onClick = { onDownload(build) }) { Text("Download archived ZIP") }
        } } }
    }
}
...
private fun formatBytes(bytes: Long?): String { if (bytes == null) return "size unknown"; if (bytes < 1024) return "$bytes B"; val kb = bytes / 1024.0; if (kb < 1024) return String.format(Locale.US, "%.1f KiB", kb); val mb = kb / 1024.0; if (mb < 1024) return String.format(Locale.US, "%.1f MiB", mb); return String.format(Locale.US, "%.2f GiB", mb / 1024.0) }
...
private fun archiveDate(filename: String): Long? { val raw = Regex("-(\\d{8,14})-").find(filename)?.groupValues?.getOrNull(1) ?: return null; return runCatching { val p = when (raw.length) { 14 -> "yyyyMMddHHmmss"; 12 -> "yyyyMMddHHmm"; 10 -> "yyyyMMddHH"; 8 -> "yyyyMMdd"; else -> return null }; SimpleDateFormat(p, Locale.US).parse(raw)?.time?.div(1000) }.getOrNull() }
